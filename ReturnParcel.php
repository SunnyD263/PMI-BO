...
function GetPNorRef($input) {
            ...
            //  unexpected parcel number
                if ($count1 === false || $count1 === null || $count1 === 0)
                    ...
                    //  parcel number from KN accounts from couriers but wrong or missing status
                            ...
                            {
                            $data = array('PARCELNO' => $stmt2['rows'][0]["PARCELNO"], 'SCAN_CODE' => '502', 'EVENT_DATE_TIME' => date('Y-m-d H:i:s'),'SERVICE' => '901', 'REFERENCE' =>  $stmt2['rows'][0]["Reference"], 'Source' => 'WHU', 'KN' => '','Customer' => '');
                            $Connection->insert("PMIdB", $data);

                            $data = array('PARCELNO' => $stmt2['rows'][0]["PARCELNO"], 'SCAN_CODE' => '502', 'EVENT_DATE_TIME' => date('Y-m-d H:i:s'),'SERVICE' => '901', 'REFERENCE' =>  $stmt2['rows'][0]["Reference"], 'Source' => 'WHU', 'KN' => '');
                            $Connection->insert("ScanBackup", $data);
    ...
    //  501 - expected parcel number      
            ...
            {
                $data = array('PARCELNO' => $stmt['rows'][0]["PARCELNO"], 'SCAN_CODE' => '501', 'EVENT_DATE_TIME' => date('Y-m-d H:i:s'),'SERVICE' => '901', 'REFERENCE' => $stmt['rows'][0]["Reference"], 'Source' => 'WHU', 'KN' => '','Customer' => '');
                $Connection->insert("PMIdB", $data);

                $data = array('PARCELNO' => $stmt['rows'][0]["PARCELNO"], 'SCAN_CODE' => '501', 'EVENT_DATE_TIME' => date('Y-m-d H:i:s'),'SERVICE' => '901', 'REFERENCE' =>  $stmt['rows'][0]["Reference"], 'Source' => 'WHU', 'KN' => '');
                $Connection->insert("ScanBackup", $data);

                $SQL=  "UPDATE [dbo].[PMIdB] SET [KN] = 'Inbound' where ([ID] = :ID)";
                $params = array(':ID' => $stmt['rows'][0]["ID"]);  
