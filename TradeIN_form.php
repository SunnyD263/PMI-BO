...
<script src="TRADEIN_form.js"></script>
</head>
<body>
    <header>
        <h1>PMI BO Tool</h1>
...
unset($row);
$_SESSION["TRADEIN_ORDITEM"] = $Order;

/******************************************************************************************************************************************************************************/
$Scan = $data['TRADEIN_SCNITEM'];
...
// if push or not button Prázdné(Empty)
    if ($_GET["Codentify"] !== "Prázdné")
        ...
        if(substr($Codentify, 0, 1) == 'T')
            {     
            if(isset($_SESSION['TRADEIN_SCNITEM']))
            {
        // check duplicite codentify in order            
            foreach ($_SESSION['TRADEIN_SCNITEM'] as $row)
                {
            ...
            $orditem = isset($_SESSION["TRADEIN_ORDITEM"]) ? json_encode($_SESSION["TRADEIN_ORDITEM"]) : "[]";
            if(!isset($_SESSION['TRADEIN_SCNITEM'])){$_SESSION['TRADEIN_SCNITEM'] = [];}
            $scnitem = isset($_SESSION['TRADEIN_SCNITEM']) ? json_encode($_SESSION['TRADEIN_SCNITEM']) : "[]";
...
//Delete scanned article
    ...
    {
    $ID=$_GET["DeleteID"];
      foreach ($_SESSION["TRADEIN_ORDITEM"] as $key => $row )
        {
        if ($row["id"] == $ID)
            {
            if ($_SESSION["TRADEIN_ORDITEM"][$key]["data"]["Checker"] !== 'Other')
                {
                $_SESSION["TRADEIN_ORDITEM"][$key]["data"]["Sum"] = $_SESSION["TRADEIN_ORDITEM"][$key]["data"]["Sum"]  - 1;
                $_SESSION["TRADEIN_ORDITEM"][$key]["data"]["ScanQuantity"] = $_SESSION["TRADEIN_ORDITEM"][$key]["data"]["ScanQuantity"]  - 1;
                }
            else
                {
                unset($_SESSION["TRADEIN_ORDITEM"][$key]);
                }
            break;
            }
        }
    ...
    $key = 0;
    foreach ($_SESSION["TRADEIN_SCNITEM"] as $key => $row )
        {
        if ($row["id"] == $ID)
            {
            unset($_SESSION["TRADEIN_SCNITEM"][$key]);
            break;
            }
        }                  
    header("Location: TRADEIN_form.php?Open=");
...
//Saving
    ...
    { 
    if (!isset($Connection)) {$Connection = new PDOConnect("DPD_DB");}
    if (isset($_SESSION["TRADEIN_ORDITEM"]))
        {
        $Orders = $_SESSION["TRADEIN_ORDITEM"];
        $Scan = $_SESSION["TRADEIN_SCNITEM"];
        foreach($Scan as $ScnIndex)
            {
        ...
        $Order = $_SESSION["TRADEIN_ORDITEM"];
        foreach($Order as $OrdIndex)
            {
...
{   
    switch ($_SESSION["Error"]) 
    {
        ...
        case "SameCDF":
            ...
            break;
        case "FormatCDF":
            echo '<span class="ErrorMsg">Neskanovali jste špatný formát codentify.</span><br><br>';
            break;
    }        
unset($_SESSION["Error"]);
}
...
/******************************************************************************************************************************************************************************/
function TRADEIN_ORDITEM()
{
    if (!isset($Connection)) {$Connection = new PDOConnect('DPD_DB');}
    if (!isset($_SESSION['TRADEIN_ORDITEM']) and empty($_SESSION['TRADEIN_ORDITEM']))
        {
        $SQL = 'SELECT [Material],[MAKTX],[EAN],[EAN_CRT],[Codentify],[ScanQuantity],[OrdQuantity],[Sum] FROM [DPD_DB].[dbo].[TRADEIN_Dvc_sum_View] WHERE ([REFERENCE] = :REFERENCE)';
        ...
        if ($count !== 0 )
            ...
            $_SESSION['TRADEIN_ORDITEM'] =  $Session;
            $rows = $Session;
            }
        }
    else
        {
        $rows = $_SESSION['TRADEIN_ORDITEM'];
        }
...
/******************************************************************************************************************************************************************************/
function TRADEIN_SCNITEM()
{
    if (isset($_SESSION['TRADEIN_SCNITEM']) and !empty($_SESSION['TRADEIN_SCNITEM']))
    {
    $rows = $_SESSION['TRADEIN_SCNITEM'];
    }
