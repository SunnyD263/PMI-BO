...
if (localStorage.getItem('Sess_ShopRec') !== null && localStorage.getItem('Sess_ShopRec') !== 'undefined') {
  ...
  Sess_ShopRec.reverse().forEach(function (record) {
...
function AddLending(clickedElementId) 
  ...
  Sess_ShopRec = JSON.parse(localStorage.getItem('Sess_ShopRec'));
  const ChngRecord  = Sess_ShopRec.findIndex(record => record.id === clickedElementId);
  Sess_ShopRec[ChngRecord].data.Type = 'LEND';
  ...
  if (userInput !== null) 
    {
      ...
      if (!isNaN(Number)) 
        ...
        Sess_ShopRec = JSON.parse(localStorage.getItem('Sess_ShopRec'));
        const ChngRecord  = Sess_ShopRec.findIndex(record => record.id === clickedElementId);
        Sess_ShopRec[ChngRecord].data.Quantity = Number;
        var jsonString = JSON.stringify(Sess_ShopRec);
        localStorage.setItem('Sess_ShopRec', jsonString);      
        FocusChng('','EAN');
        }
...
function AddBroken(clickedElementId) 
  {
    ...
    Sess_ShopRec = JSON.parse(localStorage.getItem('Sess_ShopRec'));
    const ChngRecord  = Sess_ShopRec.findIndex(record => record.id === clickedElementId);
    Sess_ShopRec[ChngRecord].data.Type = 'POSK';
...
//----------------------------------------------------------------------------------------------------
      case 'Save':
        var SendRec = JSON.parse(localStorage.getItem('Sess_ShopRec'));
        ...
        fetch('ShopRec.php', {
          ...
          body: JSON.stringify({'Shop': Shop, 'ParcelNO': ParcelNO , 'ShopRec': SendRec })
        })
          .then(response => {
            if (!response.ok) {
          ...
          EnabledChng('ALL');
            ...
            window.location.href = 'ShopRec.php?Open=';
          })
          .catch(error => {
